...
void UI::printRepositoryMenu()
{
	...
	cout << "\t 5 - Redo last operation." << endl;
...
void UI::undo()
{
	try
	{
		this->serv.undoOperation();
	}
	catch (const std::exception &e)
	{
		std::cout << "no operations in the undo stack" << std::endl;
	}
}
...
void UI::redo()
{
	try
	{
		this->serv.redoOperation();
	}
	catch (const std::exception &e)
	{
		std::cout << "no operations in the redo stack" << std::endl;
	}
}
...
void UI::updateSongFromRepo()
{
	...
	try
	{
		...
		Song newSong = Song{newArtist, newTitle, Duration{minutes, seconds}, link};

		// could also have done an add and remove operation at this level
		// tho that would probably mess with the undo/redo stack

		this->serv.updateSongFromRepository(artist, title, newSong);
	}
	catch (RepositoryException &e)
	{
		cout << e.what() << endl;
