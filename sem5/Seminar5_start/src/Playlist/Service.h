...
public:
	std::stack<std::unique_ptr<Action>> undo;
	std::stack<std::unique_ptr<Action>> redo;
	...
	*/
	...
	void updateSongFromRepository(const std::string &artist, const std::string &title, const Song &newSong);
